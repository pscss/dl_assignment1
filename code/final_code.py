...
def random_check(X, y):
    ...
    plt.imshow(img)
    print(f"Class in test data is: {print_label(cat)}")
    plt.show()
...
plt.subplot(121)
plt.title(
    f"Class in test data : {print_label(y_test[image_number])}\n"
    + f"Class predicted by model: {print_label(np.round_(test_one[0]))}"
)
